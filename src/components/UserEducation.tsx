...
const UserEducation = ({open, setOpen, dreamScore, currentScore, dateExam, exam} : stateProps) => {
    ...
    return (
      ...
      >
        ...
        >
        <img src={exit} alt="exitIcon" className='absolute right-5 top-5 opacity-60 hover:opacity-100 ease-in-out' onClick={setOpen(false)}/>
          <div>
            <h3 className="text-smrtBlue text-[32px] leading-[150%]">Текущий уровень</h3>
            <h4 className="text-transDesc text-[16px] mb-[20px] font-normal">{exam} - {dateExam}</h4>
          ...
          <button className="group relative h-12 w-[278px] overflow-hidden rounded-md bg-smrtBlue text-lg font-bold text-white justify-end text-center mx-auto inline-block" onClick={setOpen(false)}>
            Назад
